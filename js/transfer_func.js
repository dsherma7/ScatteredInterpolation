...
function reset_transfer_function() {
	...
	build_plot();  draw_legend(18);	reset_phong();
}
...
function setGUIColors() {
	...
	for (var idx=0; idx<guis.length; idx++){
		var initial_values = pointGeos[idx].attributes.value.array;
		initial_values.forEach(function(d,i){
			var rgb = getColor(idx, d);
			pointGeos[idx].attributes.alpha.array[i] = getOpacity(idx, d);
			pointGeos[idx].attributes.color.array[3*i+0] = rgb.r / 255;
			pointGeos[idx].attributes.color.array[3*i+1] = rgb.g / 255;
			pointGeos[idx].attributes.color.array[3*i+2] = rgb.b / 255;
		 });

		// Update Interpolated Values
		var interp_values = pointGeo2s[idx].attributes.value.array;
		interp_values.forEach(function(d,i){
			var rgb = getColor(idx, d);
			pointGeo2s[idx].attributes.alpha.array[i] = getOpacity(idx, d);
			pointGeo2s[idx].attributes.color.array[3*i+0] = rgb.r / 255;
			pointGeo2s[idx].attributes.color.array[3*i+1] = rgb.g / 255;
			pointGeo2s[idx].attributes.color.array[3*i+2] = rgb.b / 255;
		 });

		pointGeos[idx].attributes.alpha.needsUpdate = pointGeo2s[idx].attributes.alpha.needsUpdate = true
		pointGeos[idx].attributes.color.needsUpdate = pointGeo2s[idx].attributes.color.needsUpdate = true
	}
}
